...
si <- sessionInfo()
saveRDS(si, "scripts/conjoint_current_session_info.rds")
...
dat2$urban_rural_true <- factor(
  dat2$urban_rural_true,
  levels = c(1, 2, 3),
...
urban_only <- exp_dat %>% 
  filter(urban_rural_true == "Urban")

suburban_only <- exp_dat %>% 
  filter(urban_rural_true == "Suburban")

rural_only <- exp_dat %>% 
  filter(urban_rural_true == "Rural")
...
tbl <- data_table %>%
  ...
  collapse_rows(columns = c(1, 2), valign = "top", , row_group_label_position = "stack")
...
tbl <- data_table %>%
  ...
  collapse_rows(columns = c(1, 2, 3), valign = "top", , row_group_label_position = "stack")

save_kable(tbl, file = "tables/SI_Table_12.tex")


### Figure 5 in Main Manuscript ######################################################
#Figure 5 displays the Marginal means for clear support – defined as a support rating greater than 5 on a 7-point Likert scale – for differently designed Agrivoltaics projects among left-leaning (blue) and right-leaning (yellow) 
...
make_plot <- function(attr, show_ylabel = TRUE, show_legend = FALSE) {
  p <- ggplot(subset(mm_lf_binary_2, feature == attr), 
    ...
    xlab("") +
    coord_cartesian(ylim = c(0.325, 0.715)) +
...
title <- ggdraw() + 
  draw_label("Effects of interaction with political ideology", 
             fontface = 'bold', 
             size = 20, 
             x = 0.5, 
             hjust = 0.5)
...
### SI Figure 6 ######################################################
#SI Figure 6 displays the Marginal means for clear support – defined as a support rating greater than 5 on a 7-point Likert scale – for differently designed Agrivoltaics projects among left-leaning (blue) and right-leaning (yellow) in the NIMBY treatment group (reactangles), the control group (circles) and the overall sample (grey)
...
title <- ggdraw() + 
  draw_label("Effects of interaction with political ideology and treatment Group", 
             fontface = 'bold', 
             size = 20, 
             x = 0.5, 
             hjust = 0.5)
...
ggsave("plots/SI_Figure_6.png", plot = final_plot, width = 16, height = 15)
...
make_plot_ur <- function(attr, show_ylabel = TRUE, show_legend = FALSE) {
  p <- ggplot(subset(mm_ur, feature == attr), 
              aes(x = level, y = estimate, colour = urban_rural_true)) +
    geom_pointrange(aes(ymin = estimate - 1.96 * std.error, 
                        ymax = estimate + 1.96 * std.error), 
                    position = position_dodge(width = .2)) +
    scale_x_discrete(labels = scales::label_wrap(18)) +
    ...
    ylab(if (show_ylabel) "Marginal mean" else NULL) +
    xlab("") +
    coord_cartesian(ylim = c(0.27, 0.635)) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    geom_hline(yintercept = 0.3, lty = "dashed") +
    geom_hline(yintercept = 0.4, lty = "dashed") +
    geom_hline(yintercept = 0.5, lty = "dashed") +
    geom_hline(yintercept = 0.6, lty = "dashed") +
    scale_color_manual(name = "Degree of urbanization", 
...
title <- ggdraw() +
  draw_label(
    "Effects of interaction with degree of urbanization",
...
ggsave("plots/SI_Figure_9_urban_rural.png",
       plot = final_plot, width = 16, height = 15
)
...
title <- ggdraw() +
  draw_label(
    "Effects of interaction with degree of urbanizations",
    fontface = "bold", size = 20, x = 0.5, hjust = 0.5
  )
...
ggsave("plots/SI_Figure_10_urban_rural.png",
       plot = final_plot, width = 16, height = 15
)
...
# SI Figure 12 Average Marginal Interaction Effect (AMIE)##############################################
...
ggsave(p_treat_interactions, file = "plots/SI_Figure_12.pdf", width = 10, height = 14)
